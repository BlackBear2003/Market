...
function connect(event) {
    ...
    if(username) {
        ...
        stompClient.connect({
            token : 'eyJhbGciOiJIUzI1NiJ9.eyJqdGkiOiI3YzI0MGM5MzU0MDU0OWZlOGI2ZDMzMmJiNmY0Zjc3OSIsInN1YiI6IjEiLCJpc3MiOiJ3emwiLCJpYXQiOjE2NjkxNzg1ODQsImV4cCI6MTY2OTE4MjE4NH0.S2nr8jWuANdFofLfg0dPnNNBCAoRyIyEVkEoudt3Eio'
        }, onConnected, onError);
    }
...
function onConnected() {
    ...
    stompClient.send("/app/chat.beginChatWithOne.2",
        {}
    )
...
function sendMessage(event) {
    ...
    if(messageContent && stompClient) {
        ...
        stompClient.send("/app/chat.sendToOne", {}, JSON.stringify(chatMessage));
        messageInput.value = '';
    }
    event.preventDefault();
}
...
function onMessageReceived(payload) {
    ...
    var messageText = document.createTextNode('消息：'+message.text);
